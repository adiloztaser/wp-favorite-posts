...
function wp_favorite_posts() {
	...
	if ( false === isset( $_REQUEST['wpfpaction'] ) ) {
		return false;
	}
...
function wpfp_remove_favorite( $post_id = '' ) {
	...
	if ( false === wpfp_do_remove_favorite( $post_id ) ) {
		return false;
	}
...
function wpfp_add_to_usermeta( $post_id ) {
	...
	if ( empty( $wpfp_favorites ) or ! is_array( $wpfp_favorites ) ) {
...
function wpfp_link( $return = 0, $action = '', $show_span = 1, $args = array() ) {
	...
	if ( $show_span ) {
		$str = '<span class="wpfp-span">' . $str . '</span>';
	}
...
function wpfp_list_favorite_posts( $args = array() ) {
	...
	if ( ! empty( $user ) && wpfp_is_user_favlist_public( $user ) ) {
	...
	if ( @file_exists( STYLESHEETPATH . '/wpfp-page-template.php' ) ) {
...
function wpfp_content_filter( $content ) {
	if ( is_page() && strpos( $content, '{{wp-favorite-posts}}' ) !== false ) {
...
function wpfp_update_post_meta( $post_id, $val ) {
	...
	return update_post_meta( $post_id, WPFP_META_KEY, $new_val );
}
...
function wpfp_get_cookie() {
	if ( ! isset( $_COOKIE[ WPFP_COOKIE_KEY ] ) ) {
		return;
	}
...
function wpfp_is_user_favlist_public( $user ) {
	...
	if ( empty( $user_opts ) ) {
		return WPFP_DEFAULT_PRIVACY_SETTING;
	}
...
function wpfp_remove_favorite_link( $post_id ) {
	if ( false === wpfp_is_user_can_edit() ) {
		return;
	}
